  ...
  return (
    ...
    >
      <div className="w-full">
        <p className="text-gray-700 font-semibold text-base text-center sm:text-left truncate">{product.title}</p>
      </div>

      <div className="h-[160px] w-auto flex items-center justify-center">
        <Image src={product.thumbnail} alt={product.title} width={140} height={140} className="object-contain max-h-full" />
      </div>

      <div className="flex justify-between items-center w-full mt-4">
        <p className="text-green-600 font-semibold text-sm sm:text-base">Rs {product.price}</p>
        <motion.button
          whileTap={{ scale: 0.95 }}
